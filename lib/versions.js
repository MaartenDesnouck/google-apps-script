...
module.exports = async(identifier) => {
    ...
    try {
        ...
        const versions = await getVersions(auth, metadata.scriptId);
        for (version of versions.versions) {
            let description;
            console.log(` [${version.versionNumber}]\tdescription|${version.description}`);
        }
        process.exit(0);
    } catch (err) {
