...
module.exports = async(identifier, newProjectName) => {
    ...
    try {
        ...
        process.stdout.write(`Renaming '${identifier}' to '${newProjectName}' in your Google Drive...`);
        auth = await authenticate([]);
        const metadata = await getMetadata(auth, identifier);

        readline.clearLine(process.stdout);
        readline.cursorTo(process.stdout, 0);
        process.stdout.write(`Renaming '${metadata.title}' to '${newProjectName}' in your Google Drive...`);
        const renamed = await remoteRenameProject(auth, metadata.scriptId, metadata.title, newProjectName);
