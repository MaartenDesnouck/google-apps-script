...
async function getMetadata(auth, identifier) {
    ...
    if (files.length === 0) { // 0 results
        throw {
            message: `No project with name or id '${identifier}' found in your Google Drive ${checkbox.get('red')}\n` +
        ...
        if (result.name === identifier) {
            ...
            throw {
                message: `No exact match with name or id '${identifier}' found in your Google Drive ${checkbox.get('red')}\n` +
