...
async function getMetadata(auth, identifier) {
    ...
    if (files.length === 0) { // 0 results
        throw {
            message: `No remote project with title or id '${identifier}' found ${checkbox.get('red')}\n` +
                `Use 'gas get scripts' to show all your remote Google Apps Script projects`,
            print: true,
        };
        ...
        if (result.name === identifier) {
            ...
            throw {
                message: `No exact match with title or id '${identifier}' found ${checkbox.get('red')}\n` +
        ...
        if (exactMatches.length === 0) { // 0 results
            ...
            message += `Use 'gas rename <projectId> <newProjectName>' to rename script so they have a unique title or use the projectId as identifier\n\n`;
