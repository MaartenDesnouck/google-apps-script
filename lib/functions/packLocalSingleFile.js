...
function addLocalSingleFile(rootFolder, fileName) {
    ...
    if (!eaft.isPushable(extension, fileNameWithoutExtension, extensions)) {
        displayCheckbox('red');
        const err = `This file is unpushable to Google Drive because of an invalid extension or name`;
        console.log(err);
        throw err;
    }
    ...
    try {
        ...
        displayCheckbox('red');
        const myErr = `Can't seem to find '${fileName}' in the local filesystem`;
        console.log(myErr);
        throw myErr;
    }
...
function removeRemoteSingleFile(rootFolder, fileName) {
    ...
    if (!inRemote) {
        displayCheckbox('red');
        const err = `Can't seem to find '${fileName}' in the remote Google Apps Script file`;
        console.log(err);
        throw err;
    }
