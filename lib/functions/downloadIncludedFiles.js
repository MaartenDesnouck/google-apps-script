 ...
 * @param {object} successful - List of successful files.
        ...
        request.get(options, (getErr, response, body) => {
            if (!getErr && response.statusCode === 200) {
                fs.writeFile(path.join(includeDir, fileName), body, (writeErr) => {
                    ...
                    getAndWriteIncludedFiles(files, index, failed, successful).then((result) => {
                        resolve(result);
                        return;
                    }, (err) => {
                        reject(err);
...
function downloadIncludedFiles(included, dir) {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            getAndWriteIncludedFiles(included, 0, includeDir, [], []).then((result) => {
                resolve(result);
                return;
            }, (err) => {
                reject(err);
