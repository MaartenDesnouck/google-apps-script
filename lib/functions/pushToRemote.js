 ...
 */
function pushToRemote(auth, fileId, type, folder) {
    return new Promise((resolve, reject) => {
        ...
        let file;
        if (type === 'local') {
            file = path.join(folder, constants.META_DIR, constants.META_LOCAL);
        } else {
            file = path.join(folder, constants.META_DIR, constants.META_REMOTE);
        }
            ...
            drive.files.update({
                ...
                if (err) {
                    ...
                    drive.files.update({
                        ...
                        if (err) {
                            // Disaster recovery
                            remoteFile = path.join(folder, constants.META_DIR, constants.META_REMOTE);
