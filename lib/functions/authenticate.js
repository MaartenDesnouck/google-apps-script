    ...
    return new Promise((resolve, reject) => {
        const server = http.createServer((req, res) => {
            ...
            if (queryAsObject.code) {
                getToken(queryAsObject.code, undefined, oauth2Client).then((newOauth2Client) => {
...
function authenticate(options) {
    return new Promise((resolve, reject) => {
        ...
        const oauth2Client = new auth.OAuth2(clientId, undefined, `${redirectUrl}:${port}`);
        ...
        fs.readFile(tokenFile, 'utf8', (err, token) => {
            ...
            if (err !== null || token === '') {
                ...
                if (ttl < 10 * 1000 || options.refresh) {
                    getToken(undefined, token, oauth2Client).then((newOauth2Client) => {
                        resolve(newOauth2Client);
                        return;
                    }, (err) => {
                        getNewToken(oauth2Client, options).then((newOauth2Client) => {
