    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            for (const file of result.files) {
                const extension = eaft.getExtensionFromFiletype(file.type);
            ...
            if (!fileName) {
                // Remove all .js, .html and appsscript.json files that were not in remote.json
                const toDelete = [];
                for (const localFileName of localFiles) {
                    const localExtension = path.parse(localFileName).ext;
                    if (eaft.isPushable(localExtension, path.parse(localFileName).name) && !remoteNames.includes(localFileName) && localFileName !== constants.INCLUDE_FILE) {
