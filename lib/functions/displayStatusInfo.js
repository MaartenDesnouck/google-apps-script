...
async function displayStatusInfo(rootFolder, metadata) {
    ...
    if (addedFiles.length === 0 && modifiedFiles.length === 0 && deletedFiles.length === 0) {
        process.stdout.write(`Your local files and Google Drive for '${metadata.title}' are in sync`);
        checkbox.display(`green`);
    } else {
        process.stdout.write(`There are some difference between your local files and Google Drive for '${metadata.title}'`);
        console.log(``);
