...
async function recDependencies(dependencies, root) {
    ...
    // TODO think about converging the tree, dynamic programming; we dont need to calculate nor include everything more than once
    ...
    for (const dependency of Reflect.ownKeys(dependencies)) {
        ...
        if (!depencyList[versionKey]) {
            ...
            depencyList[versionKey] = {}; // placeholder so we know it is in progress
