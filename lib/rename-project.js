...
module.exports = async (identifier, newProjectName) => {
    ...
    try {
        ...
        process.stdout.write(`Renaming remote '${metadata.title}' to '${newProjectName}' ...`);
        const renamed = await remoteRenameProject(auth, metadata.projectId, metadata.title, newProjectName);
