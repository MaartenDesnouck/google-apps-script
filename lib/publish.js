...
module.exports = async() => {
    ...
    try {
        ...
        const packageJson = await findInProject('.', constants.PACKAGE);

        if (!packageJson.found) {
            throw {
                message: `Can't publish a package to gas-include without a gas-package.json\n'gas init' will help you get started`,
                print: true,
            };
        }

        const packed = await pack.publish(packageJson.folder);
        const result = await firebase.test(auth, packageJson.folder);
