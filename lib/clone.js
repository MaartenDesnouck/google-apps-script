...
module.exports = async (identifier) => {
    ...
    try {
        ...
        process.stdout.write(`Cloning '${identifier}' from Google Drive...`);
        ...
        process.stdout.write(`Cloning '${metadata.title}' from Google Drive...`);

        const downloaded = await downloadRemote(auth, metadata.scriptId, metadata.title, 'clone');
        const unpacked = await unpackRemote(path.join(process.cwd(), metadata.title), null);
