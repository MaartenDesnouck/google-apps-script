 ...
 *
 * @param {String} identifier - Id of the project we want to create a version for.
...
module.exports = async(identifier, options) => {
    ...
    try {
        ...
        process.stdout.write(`Creating new version for '${metadata.title}'...`);
        const version = await createVersion(auth, metadata.scriptId, options.description);

        readline.clearLine(process.stdout);
        readline.cursorTo(process.stdout, 0);
        process.stdout.write(`Created version nr. ${version.versionNumber} for '${metadata.title}'`);
